(ns talabat.main
  "Kotodama WASM entrypoint for the Talabat clean-room actor (L4) — Clojure port.
  ...
  facts (`talabat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "talabat")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"talabat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "talabat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :talabat-compat/ready)
